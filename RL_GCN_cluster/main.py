"""small graph cluster synchronization based on reinforcement learning """
...
def node_to_vector(state, feature1_dim):
    ...
    node2vec = Node2Vec(G1, feature1_dim-1, quiet=True, seed=0)
    model = node2vec.fit(window=5, min_count=1)
    vectorlist = [model.wv.get_vector(str(i)) for i in list(G1.nodes)]
    state_feature1 = torch.tensor(vectorlist)
    cluster_feature = env.node_cluster1
    state_feature = torch.cat((state_feature1, cluster_feature), 1)
...
class GCN_Actor1(torch.nn.Module):
    def __init__(self, feature1_dim, hidden1_dim, hidden2_dim, hidden3_dim):
        super().__init__()
        torch.manual_seed(12)
        self.conv1 = GCNConv(feature1_dim, hidden1_dim)
        self.conv2 = GCNConv(hidden1_dim, hidden2_dim)
        self.classifier1 = Linear(hidden2_dim, hidden3_dim)
        self.classifier2 = Linear(hidden3_dim, 1)
...
class GCN_Actor2(torch.nn.Module):
    def __init__(self, feature2_dim, hidden1_dim, hidden2_dim, hidden3_dim):
...
class ActorCritic:
    def __init__(self, feature1_dim, feature2_dim, hidden1_dim,
                 hidden2_dim, hidden3_dim, action_dim, actor1_lr, actor2_lr,
                 critic_lr, gamma, device):
        self.actor1 = GCN_Actor1(feature1_dim, hidden1_dim, hidden2_dim, hidden3_dim).to(device)
        self.actor2 = GCN_Actor2(feature2_dim, hidden1_dim, hidden2_dim, hidden3_dim).to(device)
    ...
    def update(self, transition_dict):
        ...
        #states_feature1 =[node_to_vector(states[i], feature1_dim) for i in range(len(states))]
        #states_feature2 = [add_feature(states_feature1[i], actions1[i]) for i in range(len(states))]
...
for i in range(1):
    ...
    plt.xlabel('Episodes')
    plt.ylabel('Returns')
    plt.title('Actor-Critic on graph1')
    plt.savefig('ac_gcn111.pdf')
    plt.show()
    end = time.time()
    print(end - start)
