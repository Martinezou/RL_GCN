"""small graph synchronization based on reinforcement learning"""
...
for i in range(1):
    ...
    hidden2_dim = 10
    gamma = 0.98
    device = torch.device('cpu') if torch.cuda.is_available() else torch.device('cpu') # torch.geometry has some conflict to cuda, so we use cpu to run code
    env = GraphEnv()
